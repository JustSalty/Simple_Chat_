...
unsigned long __stdcall ReceiveThreadFunction(void* pParam)
{
	...
	while(recv(clientSocket, buf, bufSize-1, 0))
    {
		...
		else
		{
			...
			string msg = " Client #" + cli->ClientId + ':' + (*buf);
			for (auto other_client : clients)   //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
			{
				if (other_client != cli) SendToClient(other_client, msg);
			}                                   //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	string msg = " Client #" + cli->ClientId; msg += " disconnected.";
...
//		FD_ZERO(&socksToCheck);
		...
		//{
			...
			string msg = " Client #" + clientsCnt; msg += " connected.";
			for (auto other_client : clients)   //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
			{
				SendToClient(other_client, msg);
			}                                   //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
